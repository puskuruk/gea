  ...
  static range(anchor, head, goalColumn, bidiLevel, assoc) {
    ...
    return head < anchor
      ...
      : SelectionRange.create(
          anchor,
          head,
          (!assoc ? 0 : assoc < 0 ? 8 /* RangeFlag.AssocBefore */ : 16) /* RangeFlag.AssocAfter */ | flags,
        )
  }
  /**
    @internal
...
class Chunk {
  ...
  between(offset, from, to, f) {
    for (
      let i = this.findIndex(from, -1e9 /* C.Far */, true), e = this.findIndex(to, 1000000000 /* C.Far */, false, i);
      i < e;
      i++
    )
...
function compare(a, startA, b, startB, length, comparator) {
  ...
  for (let boundChange = false; ; ) {
    ...
    if (point) {
      if (
        !(a.point && b.point && cmpVal(a.point, b.point) && sameValues(a.activeForPoint(a.to), b.activeForPoint(b.to)))
      )
    ...
    side +=
      block && !spec.inlineOrder
        ? side > 0
          ...
          : -4e8 /* Side.BlockBefore */
        : side > 0
          ? 100000000 /* Side.InlineAfter */
          : -1e8 /* Side.InlineBefore */
  ...
  static replace(spec) {
    ...
    if (spec.isBlockGap) {
      ...
      startSide =
        (start
          ? block
            ? -3e8 /* Side.BlockIncStart */
            : -1 /* Side.InlineIncStart */
          : 500000000) /* Side.NonIncStart */ - 1
      endSide =
        (end ? (block ? 200000000 /* Side.BlockIncEnd */ : 1 /* Side.InlineIncEnd */) : -6e8) /* Side.NonIncEnd */ + 1
    }
...
class MarkDecoration extends Decoration {
  constructor(spec) {
    ...
    super(
      ...
      end ? 1 /* Side.InlineIncEnd */ : -6e8 /* Side.NonIncEnd */,
      null,
      spec,
    )
  ...
  eq(other) {
    return (
      this == other ||
      (other instanceof MarkDecoration && this.tagName == other.tagName && attrsEq(this.attrs, other.attrs))
    )
  }
  range(from, to = from) {
  ...
  resolveBlock(pos, side) {
    ...
    this.blockTiles((tile, off) => {
      ...
      if (pos >= off && pos <= end) {
        ...
        if (
          (off < pos || (pos == end && (side < -1 ? tile.length : tile.covers(1)))) &&
        ...
        if (
          (end > pos || (pos == off && (side > 1 ? tile.length : tile.covers(-1)))) &&
      ...
      for (let i = 0, off = 0; i < tile.children.length && off <= pos; i++) {
        ...
        if (end >= pos) {
          if (child.isComposite()) {
            ...
            (!after || (after.isHidden && (side > 0 || (forCoords && onSameLine(after, child))))) &&
    ...
    if (found) {
      ...
      if (this.dom.contains(tile.dom)) {
        ...
        return tile.domPosFor(
          offset,
          tile.flags & 16 /* TileFlag.Before */ ? 1 : tile.flags & 32 /* TileFlag.After */ ? -1 : side,
        )
      }
...
class WidgetTile extends Tile {
  ...
  coordsInWidget(pos, side, block) {
    ...
    if (block) {
      ...
      if (!rects.length) return null
      let fromBack =
        this.flags & 16 /* TileFlag.Before */ ? true : this.flags & 32 /* TileFlag.After */ ? false : pos > 0
...
class TileBuilder {
  ...
  addText(text, marks, openStart, tile) {
    ...
    if (
      prev &&
      prev.isText() &&
      !((prev.flags & 8) /* TileFlag.Composition */) &&
      prev.length + text.length < 512 /* C.Chunk */
    ) {
      ...
      parent.append(
        tile || TextTile.of(text, (_a = this.cache.find(TextTile)) === null || _a === void 0 ? void 0 : _a.dom),
      )
    }
  ...
  addInlineWidget(widget, marks, openStart) {
    ...
    let noSpace =
      this.afterWidget &&
      widget.flags & 48 /* TileFlag.PointWidget */ &&
  ...
  addLineStart(attrs, dom) {
    ...
    let tile = LineTile.start(
      attrs,
      dom || ((_a = this.cache.find(LineTile)) === null || _a === void 0 ? void 0 : _a.dom),
      !!dom,
    )
    ...
    for (let i = marks.length - 1; i >= 0; i--) {
      ...
      if (openStart > 0 && (last = parent.lastChild) && last instanceof MarkTile && last.mark.eq(mark)) {
        ...
        let tile = MarkTile.of(
          mark,
          (_a = this.cache.find(MarkTile, (m) => m.mark.eq(mark))) === null || _a === void 0 ? void 0 : _a.dom,
        )
        parent.append(tile)
        parent = tile
        openStart = 0
  ...
  endLine() {
    if (this.curLine) {
      ...
      if (
        ...
        this.curLine.append(
          this.cache.findWidget(BreakWidget, 0, 32 /* TileFlag.After */) ||
    ...
    for (let wrap of this.wrappers) {
      ...
      if (wrap.from < this.pos && last instanceof BlockWrapperTile && last.wrapper.eq(wrap.wrapper)) {
        ...
        let tile = BlockWrapperTile.of(
          wrap.wrapper,
          (_a = this.cache.find(BlockWrapperTile, (t) => t.wrapper.eq(wrap.wrapper))) === null || _a === void 0
            ? void 0
            : _a.dom,
        )
        parent.append(tile)
  ...
  blockPosCovered() {
    ...
    return (
      last != null &&
      !last.breakAfter &&
      (!last.isWidget() || (last.flags & (32 /* TileFlag.After */ | 128) /* TileFlag.IncEnd */) > 0)
    )
  }
  getBuffer(side) {
      ...
      for (let i = 0, pass = 0; ; i++) {
        ...
        if (
          ...
          if (
            tile.widget == widget &&
            tile.length == length &&
            (tile.flags & (496 /* TileFlag.Widget */ | 1) /* TileFlag.BreakAfter */) == flags
          ) {
            ...
            let widget =
              to > 0 || from < tile.length
                ? WidgetTile.of(
                    ...
                    this.view,
                    to - from,
                    tile.flags & 496 /* TileFlag.Widget */,
      ...
      point: (from, to, deco, active, openStart, index) => {
        if (deco instanceof PointDecoration) {
          ...
          if (openStart > active.length) {
            ...
            let tile =
              this.cache.findWidget(widget, to - from, flags) || WidgetTile.of(widget, this.view, to - from, flags)
            if (deco.block) {
  ...
  getCompositionContext(text) {
    ...
    for (let parent = text.parentNode; ; parent = parent.parentNode) {
      ...
      if (tile instanceof MarkTile) marks.push(tile)
        ...
        marks.push(
          MarkTile.of(
            new MarkDecoration({ tagName: parent.nodeName.toLowerCase(), attributes: getAttrs(parent) }),
            parent,
          ),
        )
    }
...
function widgetFlags(deco) {
  let flags = deco.isReplace
    ? (deco.startSide < 0 ? 64 /* TileFlag.IncStart */ : 0) | (deco.endSide > 0 ? 128 /* TileFlag.IncEnd */ : 0)
    : deco.startSide > 0
      ? 32 /* TileFlag.After */
      : 16 /* TileFlag.Before */
...
class DocView {
  ...
  updateInner(changes, composition) {
    ...
    observer.ignore(() => {
      ...
      if (
        track &&
        (track.written || observer.selectionRange.focusNode != track.node || !this.tile.dom.contains(track.node))
      )
        this.forceSelection = true
  ...
  scrollIntoView(target) {
    ...
    let rect = this.coordsAt(
        range.head,
        (_a = range.assoc) !== null && _a !== void 0 ? _a : range.empty ? 0 : range.head > range.anchor ? -1 : 1,
      ),
      other
    if (!rect) return
...
function moveVertically(view, start, forward, distance) {
  ...
  let startCoords = view.coordsAtPos(
    startPos,
    start.assoc || ((start.empty ? forward : start.head == start.from) ? 1 : -1),
  )
      ...
      if (
        (tile && nextTile
          ...
          : (tile ? tile.breakAfter : isBlockElement(cur)) ||
            (isBlockElement(next) &&
              (cur.nodeName != 'BR' || (tile === null || tile === void 0 ? void 0 : tile.isWidget())) &&
  ...
  if (domChange.bounds) {
    ...
    if (
      ...
      cmp.slice(sel.to - from) ==
        domChange.text.slice((selEnd = domChange.text.length - (cmp.length - (sel.to - from))))
    ) {
...
function applyDefaultInsert(view, change, newSel) {
  ...
  if (inAtomic > -1) {
    ...
    if (
      ...
      (view.inputState.composing >= 0 || view.inputState.compositionPendingChange) &&
      change.to <= sel.to + 10 &&
      change.to >= sel.to - 10
    ) {
...
function basicMouseSelection(view, event) {
  ...
  return {
    ...
    get(event, extend, multiple) {
      let cur = view.posAndSideAtCoords({ x: event.clientX, y: event.clientY }, false),
        removed
    ...
    this.handlers.textupdate = (e) => {
      ...
      if (!view.state.readOnly) {
        ...
        applyDOMChangeInner(
          view,
          change,
          EditorSelection.single(this.toEditorPos(e.selectionStart, newLen), this.toEditorPos(e.selectionEnd, newLen)),
        )
      }
...
class EditorView {
  ...
  static findFromDOM(dom) {
    ...
    return (
      ((_a = tile === null || tile === void 0 ? void 0 : tile.root) === null || _a === void 0 ? void 0 : _a.view) ||
      null
    )
  }
}
...
const bracketMatcher = /*@__PURE__*/ ViewPlugin.fromClass(
  class {
    constructor(view) {
      this.paused = false
...
const bracketMatchingUnique = [bracketMatcher, baseTheme$1]
/**
...
class Stack {
  ...
  canShift(term) {
    for (let sim = new SimulatedStack(this); ; ) {
      let action =
        this.p.parser.stateSlot(sim.state, 4 /* ParseState.DefaultReduce */) || this.p.parser.hasAction(sim.state, term)
    ...
    if (!(side < 0 ? cursor.childBefore(pos) : cursor.childAfter(pos)))
      for (;;) {
        if ((side < 0 ? cursor.to < pos : cursor.from > pos) && !cursor.type.isError)
          return side < 0
            ? Math.max(0, Math.min(cursor.to - 1, pos - 25 /* Lookahead.Margin */))
...
const parser = LRParser.deserialize({
  ...
  specialized: [
    { term: 327, get: (value) => spec_identifier[value] || -1 },
    { term: 343, get: (value) => spec_word[value] || -1 },
