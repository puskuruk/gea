...
export default class Component<P = Record<string, any>> extends Store {
  ...
  mountCompiledChildComponents_() {
    ...
    seen.forEach((child) => {
      ...
      if (typeof child.__adoptListItems === 'function') child.__adoptListItems()
   ...
   * `this.__child()` calls inside `.map()`.  During SSR hydration, `template()`
   * is never called, so the arrays stay empty.  When a store change later
   * triggers `__reconcileList`, the empty `oldItems` array causes every item
   * to be re-created — duplicating the server-rendered DOM.
   */
  __adoptListItems(): void {
    const adopted = this.__childComponents.filter(
      (c) => c.__geaItemKey != null && c.rendered_ && c.element_,
    )
    if (adopted.length === 0) return

    // Sort by DOM order
    adopted.sort((a, b) => {
      if (!a.element_ || !b.element_) return 0
      const pos = a.element_.compareDocumentPosition(b.element_)
      if (pos & Node.DOCUMENT_POSITION_FOLLOWING) return -1
      if (pos & Node.DOCUMENT_POSITION_PRECEDING) return 1
      return 0
    })

    // Use the raw store target (bypassing the reactive proxy) so that
    // populating tracking arrays doesn't generate change notifications
    // that cause duplicate reconciliation during Store.flushAll().
    const raw = ((this as Record<string, unknown>).__raw ?? this) as Record<string, unknown>

    // Path 1: Populate _*Items instance properties (getter-backed arrays)
    ...
    // Path 2: Populate __observeList config.items (direct store arrays)
    const configs = raw.__geaListConfigs
